...
<body>
	<?php
		if (!empty($_GET["id_lavorazione"])) {
			$xml_file_name = get_order_xml_for_print($_GET['id_lavorazione'], $mysqli, $App_BaseServerPath, 0);
			if ($xml_file_name!='-1') {
				$xml_file_extension = '.xml';
				$pdf_file_extension = '.pdf';
				exec($fop_folder . '/' . $fop_command . ' -xml ' . $App_BaseServerPath . '/print_output/' . $xml_file_name . $xml_file_extension . ' -xsl ' . $App_BaseServerPath . '/print_template/order_print.xslt -pdf ' . $App_BaseServerPath . '/print_output/' . $xml_file_name . $pdf_file_extension);
