	...
	<div class="bg-outer">
		<div class="container gray-dark">
			...
			<div class="page-header">
				<h1>Articoli <small>Anagrafiche</small></h1>
			</div>
			<form data-toggle="validator" id="search_form" role="form" action="item_list.php" method="post" name="search_form" data-disable="true">
				<!-- ***************************************************** -->
				<!--                  FILTRI DI RICERCA                    -->
				<!-- ***************************************************** -->
				<div class="col-md-12">
								...
								<div class="form-group col-md-3">
									...
									<div class="input-group">
										<div class="input-group-btn">
											<button type="button" class="btn btn-default dropdown-toggle input-sm" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><?php echo !empty($_POST["id_articolo_searchop"])?$_POST["id_articolo_searchop"]:'=' ?> <span class="caret"></span></button>
											<ul class="dropdown-menu" id="id_articolo_searchop_list">
												<li><a href="#">=</a></li>
												<li><a href="#">>=</a></li>
												<li><a href="#"><=</a></li>
										...
										<input type="number" class="form-control input-sm" id="id_articolo" name="id_articolo" value="<?php echo !empty($_POST["id_articolo"])?$_POST["id_articolo"]:'' ?>">
										<input type="hidden" id="id_articolo_searchop" name="id_articolo_searchop" value="<?php echo !empty($_POST["id_articolo_searchop"])?$_POST["id_articolo_searchop"]:'=' ?>">
									</div>
								</div>
								<!-- ***************************************************** -->
								<!--                  DESCRIZIONE                          -->
				...
				<div class="col-md-12">
					...
					<button id="new-button" type="button" class="btn btn-primary btn-sm" onClick="javascript:windowpop('item_form.php?action=new',800,600)" title="Nuovo articolo" data-toggle="tooltip"><span class="glyphicon glyphicon-plus"></span></button>
				</div>
				<br>
				<div class="table-responsive col-md-12">
					<input type="hidden" id="sort_index" name="sort_index" value="<?php echo !empty($_POST["sort_index"])?$_POST["sort_index"]:'id_articolo' ?>">
					...
					<table id="table_results" class="table table-hover table-striped">
						<thead>
							<tr>
								<th onClick="javascript:tableHeaderClick('A.id_articolo')">ID Articolo <?php echo !empty($_POST["sort_index"])?($_POST["sort_index"]=='A.id_articolo'?(!empty($_POST["sort_dir"])?($_POST["sort_dir"]=='ASC'?'<span class="glyphicon glyphicon-sort-by-attributes"></span>':'<span class="glyphicon glyphicon-sort-by-attributes-alt"></span>'):''):''):'' ?></th>
								<th onClick="javascript:tableHeaderClick('A.descrizione')">Descrizione <?php echo !empty($_POST["sort_index"])?($_POST["sort_index"]=='A.descrizione'?(!empty($_POST["sort_dir"])?($_POST["sort_dir"]=='ASC'?'<span class="glyphicon glyphicon-sort-by-attributes"></span>':'<span class="glyphicon glyphicon-sort-by-attributes-alt"></span>'):''):''):'' ?></th>
						...
						<tbody>
							...
							<?php
								...
								$sort_index = !empty($_POST["sort_index"])?$_POST["sort_index"]:'A.id_articolo';
								$sort_dir = !empty($_POST["sort_dir"])?$_POST["sort_dir"]:'ASC';
								$items_coll = get_item_list(
																!empty($_POST["id_articolo"])?$_POST["id_articolo"]:'',
																!empty($_POST["id_articolo_searchop"])?$_POST["id_articolo_searchop"]:'=',
							...
							?>
									<tr id="tr_id_1" class="tr-class-1">
										<td><?php echo $item["id_articolo"] ?></td>
										<td><?php echo htmlentities($item["descrizione"]) ?></td>
										<td><?php echo htmlentities($item["categoria"]) ?></td>
										<td>
											<button id="edit-button" type="button" class="btn btn-primary btn-sm" onClick="javascript:windowpop('item_form.php?action=edit&id_articolo=<?php echo $item["id_articolo"] ?>',800,600)" title="Modifica" data-toggle="tooltip"><span class="glyphicon glyphicon-pencil"></span></button>
											<button id="delete-button" type="button" class="btn btn-primary btn-sm" onClick="javascript:windowpop('item_form.php?action=delete&id_articolo=<?php echo $item["id_articolo"] ?>',800,500)" title="Elimina" data-toggle="tooltip"><span class="glyphicon glyphicon-trash"></span></button>
										</td>
									</tr>
							<?php
								}
